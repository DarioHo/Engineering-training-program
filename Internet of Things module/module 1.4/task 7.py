...
topic_state_eme = "PC304/traffic_light/emergency" 
...
def Blinking_red_green(duration, time_blink):
      j = 0
      while j < duration:
            light_reset()
            time.sleep(time_blink)
...
def light_show():
      ...
      if emergency == True:
            Blinking_red_green(10,0.2)
            mqttc.publish(topic_state_eme, 1, qos=1, retain=False)
            print (emergency)
